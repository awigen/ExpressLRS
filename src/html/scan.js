...
function getPwmFormData() {
  ...
  while (inField = _(`pwm_${ch}_ch`)) {
    ...
    const raw = (narrow << 19) | (mode << 15) | (invert << 14) | (inChannel << 11) | (failsafe);
...
function updatePwmSettings(arPwm, allowDshot) {
  ...
  const htmlFields = [`<div class="mui-panel"><table class="pwmtbl mui-table"><tr><th class="mui--text-center">Output</th><th>Mode</th><th>Input</th><th class="mui--text-center">Invert?</th><th class="mui--text-center">750us?</th>
  <th>Failsafe</th>
  <th>Limit Min</th>
  <th>Limit Max</th>
  </tr>`];
  arPwm.forEach((item, index) => {
    ...
    htmlFields.push(`<tr><th class="mui--text-center">${index+1}</th>
            ...
            <td><div class="mui-textfield"><input id="pwm_${index}_fs" value="${failsafe}" size="6"/></div></td>
            <td><div class="mui-textfield"><input id="pwm_${index}_limit_min" value="${item.limits.min}" size="6"/></div></td>
            <td><div class="mui-textfield"><input id="pwm_${index}_limit_max" value="${item.limits.max}" size="6"/></div></td>
            </tr>`);
  });
